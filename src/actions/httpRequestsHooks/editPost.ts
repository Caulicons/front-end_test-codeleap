...
const useEditPost = () => {
   ...
   return async (postToEdit: newPost) => {
      ...
      return codeleapHTTP.patch(`${postSelected?.id}/`, postToEdit)
         .then(res => {

            if (res.status === 200) {
               dispatch(editPostInStore(res.data));
               dispatch(editPostPopUp());
               showNotification({ text: 'Post edited successfully!', type: 'success' });
            };
         })
         .catch(err => {
            console.log(err);
            showNotification({ text: 'Unable to edit post. Try again later.', type: 'failed' });
         });
      ;
   };
};
